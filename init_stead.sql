...
DROP COLUMN  normdoc,
DROP COLUMN counter,
ADD COLUMN actstatus INTEGER,
ADD COLUMN level  INTEGER,
ADD COLUMN kod_t_st INTEGER
;

ALTER TABLE TMP_STEAD RENAME COLUMN  steadid TO id;
ALTER TABLE TMP_STEAD RENAME COLUMN  steadguid TO guid;
ALTER TABLE TMP_STEAD RENAME COLUMN  "number" TO name;
...
-- cast column TMP_STEAD .divtype to int

    alter table TMP_STEAD  rename column divtype to divtype_x;
    alter table TMP_STEAD  add column divtype int;
    update TMP_STEAD  set divtype = divtype_x::int;
    alter table TMP_STEAD  drop column divtype_x;
...
UPDATE TMP_STEAD  SET level = 75 ;
UPDATE TMP_STEAD  SET kod_t_st = 7501 ;
UPDATE TMP_STEAD  SET actstatus = 0 ;
UPDATE TMP_STEAD  SET actstatus = 1 where  nextid is null;
