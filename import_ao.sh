...
do
    ...
    if [[ "$TABLE" != "$TBL_TO_LOAD" ]]; then
        ...
        psql postgresql://$POSTGRES_USER:$POSTGRES_PASSWORD@$POSTGRES_HOST:$POSTGRES_PORT/$POSTGRES_DB -c "DROP TABLE IF EXISTS TMP_AO;"
        psql postgresql://$POSTGRES_USER:$POSTGRES_PASSWORD@$POSTGRES_HOST:$POSTGRES_PORT/$POSTGRES_DB -c "CREATE TABLE TMP_AO  AS SELECT * FROM public.AO2 WHERE 0 = 1;"
